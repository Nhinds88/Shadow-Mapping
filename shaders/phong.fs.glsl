...
void main(void) {
    ...
    float lightDepth = (((lightSpaceNDC.z - (-1.0)) * (1.0 - 0.0)) / (1.0 - (-1.0)) + 0.0);
    ...
    // todo #9
    // gl_FragColor = vec4(lightDepth, lightDepth, lightDepth, 1.0); // vec4(finalColor, 1.0); // remove this when you are ready to add shadows
